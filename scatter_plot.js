function scatter_plot(dat, X,Y,markersize,
                      ColorData,
                      axis_key,
                      title="",
                      legend=[],
                      ...
                      margin = 100)
{
    ...
    let rScale= d3.scaleLinear().domain(d3.extent(markersize,function (d){return d})).range([4,12])
    //let colorScale= d3.scaleLinear().domain(d3.extent(ColorData)).range(['steelblue','brown'])
    function colorScale(ColorData){
        if(ColorData == 0){
            return 'steelblue'
        }
        else if(ColorData == 1){
    ...
    axis.selectAll('.markers')
        ...
        .append('circle').attr("r",function (d){return rScale(d.flipper_length_mm)})//.attr("r",markersize)
        .style("fill",function (d,i){return colorScale(ColorData[i])})
        .on("mouseover", function(d){
            div.transition()		
                .duration(200)		
                .style("opacity", .9);		
            div	.html("Flipper length: " + d.path[0].__data__.flipper_length_mm)	
    ...
    if (legend.length>0){
        legend.forEach(
            ...
            lgnd.append('rect').attr('width',function (d){return 40})
                ...
                .attr("class",d)
            lgnd.append('text').attr("class","legend").attr("dx","-140").attr("dy","30").text(d).style("font-size", "34px")
        })
    }
